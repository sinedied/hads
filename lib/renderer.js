  ...
  static renderIndexLevel(index, level) {
    ...
    keys.forEach(key => {
      ...
      if (typeof value === 'string') {
        ...
        const indexFileIndex = valueKeys.findIndex(k => ROOT_FILES_INDEX[k]);
        if (~indexFileIndex) {
          const indexFileKey = valueKeys[indexFileIndex];
          const indexFileValue = value[indexFileKey];
          const link = Renderer.encodeFileLink(indexFileValue);
          content += `- [${humanize(Renderer.trimNumberPrefix(key))}](/${link})\n`;
        } else {
